...
function storyMatchesCompany(story, company) {
  ...
  if(relevance>=.38 && ((name&&hay.includes(name))||(ticker&&hay.includes(ticker)))) return true;
  return false;
}
...
function storyMatchesTheme(story, theme) {
  ...
  const bodyHits=terms.filter(term=>term&&hay.includes(term.toLowerCase()));
  const titleHits=terms.filter(term=>term&&title.includes(term.toLowerCase()));
  return bodyHits.length>=2 || titleHits.length>=1;
}
...
function companyCard(c, type="signal") {
  ...
  const score=discovery ? c.discovery_score : attention ? (c.market_attention_score ?? c.signal_score) : c.signal_score;
...
function getFiltered(kind) {
  ...
  const list=(state.rankings[key]||[]).slice(0,12).map(companyByTicker).filter(Boolean);
  const q=$("searchInput").value.trim().toLowerCase();
  const min=Number($("minScore").value);
  const sector=$("sectorFilter").value;
  const cat=$("catalystFilter").value;
  return list.filter(c=>{
    const score=kind==="discovery" ? (c.discovery_score||0) : (c.market_attention_score ?? c.signal_score ?? 0);
...
function attachCards() {
  document.querySelectorAll(".company-card").forEach(card=>{
    ...
    card.addEventListener("keydown",e=>{
      if(e.key==="Enter"||e.key===" "){e.preventDefault();openCompany(card.dataset.ticker);}
    });
  });
}
...
function openCompany(ticker) {
  ...
  $("dialogContent").innerHTML=`
    <div class="detail-header"><div class="detail-title-row"><div><p class="eyebrow">${esc(c.ticker)}${c.company_name?` · ${esc(c.company_name)}`:""}</p><h2>${esc(c.primary_catalyst||"Credible company development")}</h2></div></div>
...
function renderAll(){renderRankings();}
